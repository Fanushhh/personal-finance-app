...
export const BudgetForm = ({closeModal}) => {
    ...
    const {data, isLoading, isError, error} = useQuery({ queryKey: ['budgets'], queryFn: getBugets });
        ...
        <div className="relative flex-col rounded-xl gap-4 bg-white p-6  shadow-lg">
                <div className="flex justify-between items-start mb-3">
                    <div className="max-w-[500px]">
                        ...
                        <p className="preset-4 text-[var(--beige-500)]">Choose a category to set a spending budget. These categories can help you monitor spending.</p>
                    </div>
                       ...
                       <select name="budgetCategory" id="budgetCategory">
                        {
                            updatedCategories.map((option) => {
                                return <option key={crypto.randomUUID()} value={option.value}>{option.label}</option>
                            })
                        }
                       </select>
                       {!message?.success && <p className="text-red-500">{message?.message.budgetCategory}</p>}
                    </div>
                    
                    <div>
                        <label htmlFor="maxSpend">Maximum Spend</label>
                        <input placeholder="$ e.g. 2000" type="text" id="maxSpend" name="maxSpend" autoComplete="off"/>
                        {!message?.success && <p className="text-red-500">{message?.message.maxSpend}</p>}
                    </div>
